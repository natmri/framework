...
import type { CancelablePromise, IDisposable } from '@natmri/core'
import { CancellationError, CancellationToken, CancellationTokenSource, DisposableStore, Emitter, Event, EventMultiplexer, Relay, combinedDisposable, createCancelablePromise, dispose, getRandomElement, isFunction, isUndefinedOrNull, timeout, toDisposable } from '@natmri/core'
import { VSBuffer } from './helper/buffer'
import { isUpperAsciiLetter } from './helper/char'
import { revive } from './helper/marshalling'
...
export namespace ProxyChannel {
  ...
  type StaticEvent<T> = T extends Event<any> ? T : Event<ReturnType<T>>
  type DynamicEvent<T> = T extends (...args: any[]) => Event<any> | Promise<Event<any>> ? T : (...args: any[]) => Event<ReturnType<T>> | Promise<Event<ReturnType<T>>>
  export type ProxyService<T extends Record<string, any>> = {
    [K in keyof T]: K extends string ? StartsWith<K, 'on'> extends true ? StartsWith<K, 'onDynamic'> extends true ? DynamicEvent<T[K]> : StaticEvent<T[K]> :
  ...
  export function toService<T extends object>(channel: IChannel, options?: ICreateProxyServiceOptions): ProxyService<T> {
    const disableMarshalling = options && options.disableMarshalling
